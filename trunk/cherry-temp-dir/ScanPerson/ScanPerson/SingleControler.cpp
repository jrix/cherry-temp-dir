...
initStatus SingleControler::init(){
	...
	{	
		...
		XnDepthPixel* d_x=new XnDepthPixel[blockSize];
		XnDepthPixel* d_y=new XnDepthPixel[blockSize];
		int len=0;
		for(int i=0;i<sub_y;i++){
			for(int j=0;j<sub_x;j++){
				...
				d_x[len]=(XnDepthPixel)bigX;
				d_y[len]=(XnDepthPixel)bigY;
				int base=len*3;
				int base1=base+1;
				int base2=base+2;
				depthPts[base]=&d_x[len];
				depthPts[base1]=&d_y[len];
				depthPts[base2]= const_cast <XnDepthPixel*>(&depPix[idx]);
				clorPts[base]=const_cast <XnUInt8*>(&imgPix[idx*3]);
				clorPts[base1]=const_cast <XnUInt8*>(&imgPix[idx*3+1]);
				clorPts[base2]=const_cast <XnUInt8*>(&imgPix[idx*3+2]);
				len++;
			}
		}
		return this->ini_stus;
