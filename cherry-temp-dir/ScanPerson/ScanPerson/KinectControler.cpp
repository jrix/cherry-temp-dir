...
KinectControler::KinectControler():_devData(NULL),_vrmlData(NULL),_xStep(1),_yStep(1),blockSize(0){

}
KinectControler::KinectControler(int x_step,int y_step):_devData(NULL),_vrmlData(NULL),_xStep(x_step),_yStep(y_step),blockSize(0){
	if(!_xStep){
		_xStep=1;
	}
	if (_yStep)
...
XnUInt32 KinectControler::getXStep(){
	return this->_xStep;
}
XnUInt32 KinectControler::getYStep(){
	return this->_yStep;
}

//######## ######## ######## ######## ######## 
SingleControler::SingleControler(){}
SingleControler::SingleControler(int x,int y):KinectControler(x,y)
{
	
}
SingleControler::~SingleControler(){}

HRESULT SingleControler::init(){
	int xStep=getXStep();
	int yStep=getYStep();
	...
	int small_x=(int)(_devData[0].xres/xStep);
	int small_y=(int)(_devData[0].yres/yStep);
	blockSize=(small_x*small_y);
	return S_OK;
